...
class UserRepository:
    ...
    async def get_user_questions(self, user_id: int):
        questions = await self.session.execute(select(Question).where(Question.author_id == user_id))
        questions = questions.scalars().all()
        return questions

    async def get_user_answers(self, user_id: int):
        answers = await self.session.execute(select(Answer).where(Answer.author_id == user_id))
        answers = answers.scalars().all()
        return answers
    
    async def get_public_user_questions(self, user_id: int):
        questions = await self.session.execute(select(Question).where(and_(Question.author_id == user_id, Question.anonymous != True)))
        questions = questions.scalars().all()
        return questions
    
    async def get_public_user_answers(self, user_id: int):
        answers = await self.session.execute(select(Answer).where(and_(Answer.author_id == user_id, Answer.anonymous != True)))
        answers = answers.scalars().all()
        return answers
